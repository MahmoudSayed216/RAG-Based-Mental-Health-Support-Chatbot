...
class LLMCaller:
    ...
    def _initialize_LLM(self):
        ...
        logger.info(
            "Initialized LLMCaller [%s] with model: %s", self.identifier, model_name
        )
    ...
    def call(self, arguments: dict):
        ...
        logger.debug(
            "[%s] Sending prompt to LLM (length=%d)", self.identifier, len(prompt)
        )
        ...
        logger.info(
            "[%s] LLM call completed (output length=%d)", self.identifier, len(output)
        )
        return output
